...
def evaluate(world, display, image, detected_objects_per_frame, view_width, view_height, show_ground_truth_bbox_screen,
 important_region):

    true_pos_ML_per_frame, true_neg_ML_per_frame, false_pos_ML_per_frame, false_neg_ML_per_frame = 0, 0, 0, 0
    true_pos_ML_imp_region_per_frame, true_neg_ML_imp_region_per_frame, false_pos_ML_imp_region_per_frame, false_neg_ML_imp_region_per_frame = 0, 0, 0, 0
    ...
    for actor in world.world.get_actors():
        ...
        if actor.type_id.startswith('walker'):
            ...
            if real_bbox is not None:
                ...
                # evaluation regarding an important region
                    ...
                    # NO real pedestrian entered in the important region
                        ...
                        for det_object in detected_objects_per_frame:
                            if det_object is not None:
                                ...
                                # search for a predicted pedestrian that entered in the important region ...
                                #... (when actually there is no real pedestrian on that region)
                                if is_rect_overlap(important_region, predicted_bbox):
                                ...
                                #there is NO real pedestrian in the important region and the ML correctly did not detected anything (true negative)
                                    ...
                                    true_neg_ML_per_frame+=1

            #################### ending evaluating the predicted bbox regarding the real bbox ####################

    return true_pos_ML_per_frame, true_neg_ML_per_frame, false_pos_ML_per_frame, false_neg_ML_per_frame