...
def ice(x, severity=1):
    ...
    return overly_img(x, foreground_image_path[severity-1], alpha[severity-1])
...
def rain_mask(x):
    ...
    alpha = 0.15
    return overly_img(x, foreground_image_path, alpha)
...
def broken_lens(x, severity=1):
    ...
    x = Image.fromarray((x * 255).astype(np.uint8))
    foreground_image_path_list = {'src/img/broken1.png', 'src/img/broken2.png'}
...
def apply_novelty(RGBA_x, frame_num_novelty):
    ...
    foreground_image_path = ['src/img/fallen_tree.png']# 'img/crashed_car.png'
...
def apply_anomaly(RGBA_x, frame_num_novelty):
    ...
    foreground_image_path = ['src/img/crashed_car.png']# it's an array if you want to add more images
    foreground_image = foreground_image_path[0]    
...
def apply_threats(img, aug_type, severity):
    ...
    if aug_type == 'shifted_pixel':
        ...
        x = Image.fromarray((img*255).astype(np.uint8))
        foreground_image_path = 'src/img/dirty.png'
        alpha = severity#[0.4, 0.5, 0.6, 0.7, 0.8]
        img = overly_img(x, foreground_image_path, alpha)
        return img

    elif aug_type == 'sun_flare':
        flare_roi = (0.60, 0.60, 0.75, 0.75)
        ...
        #fill_values = {0, 255, 122}#[0, 255, 0, 255, 122]
