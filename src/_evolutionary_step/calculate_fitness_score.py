...
def init_task(data_path, weights='yolov5s.pt', task='val', conf_thres=0.001):
    check_requirements(requirements=ROOT / 'requirements.txt', exclude=('tensorboard', 'thop'))
    
    prefix_yaml = 'transformed_coco128.yaml'
    data = data_path.split("/")
    data = '{}/{}'.format(data[0]+'/'+data[1], prefix_yaml)

    if task in ('train', 'val', 'test'):  # run normally
        if conf_thres > 0.001:  # https://github.com/ultralytics/yolov5/issues/1466
            LOGGER.info(f'WARNING: confidence threshold {conf_thres} >> 0.001 will produce invalid mAP values.')
        return(run(data, weights, task=task, conf_thres=conf_thres))