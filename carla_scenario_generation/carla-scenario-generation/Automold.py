...
def flare_source(image, point,radius,src_color):
    ...
    for i in range(num_times):
        ...
        alp=alpha[num_times-i-1]*alpha[num_times-i-1]*alpha[num_times-i-1] + 0.8
...
def add_sun_flare(image,flare_center=-1, angle=-1, no_of_flare_circles=8,src_radius=200, src_color=(255,255,255)):
    verify_image(image)
    if(angle!=-1):
...
#     cv2.ellipse(overlay, center =center,box=None,color =src_color)
    ...
    #print('=========', overlay, (center, (width,height), 0), src_color, -1)
    cv2.ellipse(overlay, (center, (width,height), 0), src_color, -1)
...
def add_manhole(image,center=-1,color=(120,120,120),height=1,width=1, type='closed'): ##function to flip the image on horizontal axis
    ...
    if(is_list(image)):
        ...
        for img in image_list:
            ...
            center_t=center
            if height==1:
                height_t=int(img.shape[0]//25)
            if width==1:
                width_t=int(img.shape[0]*3//25)
            if center==-1:
                center_t= (img.shape[0]-100, int(img.shape[1]//2))
        ...
        center_t=center
        if height==1:
            height_t=int(image.shape[0]//25)
        if width==1:
            width_t=int(image.shape[0]*3//25) 
        if center==-1:
            center= (image.shape[0]-100, int(image.shape[1]//2))
        image_RGB= manhole_process(image,center,height_t,width_t,color)
    return image_RGB
