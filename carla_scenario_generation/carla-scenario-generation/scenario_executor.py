...
def draw_bboxes(world, 
    ...
    for detection in detections:
        ...
        np.random.seed(hash(detection['label']) % (2 ** 32 - 1))
        color = np.random.uniform(low=0, high=255, size=(3))

        # Draw the bounding box
        pygame.draw.line(bb_surface, color, (bbox[0], bbox[1]), (bbox[2], bbox[1]))
        pygame.draw.line(bb_surface, color, (bbox[2], bbox[1]), (bbox[2], bbox[3]))
        pygame.draw.line(bb_surface, color, (bbox[2], bbox[3]), (bbox[0], bbox[3]))
        pygame.draw.line(bb_surface, color, (bbox[0], bbox[3]), (bbox[0], bbox[1]))

        #detection_line = yolobbox2bbox(bbox[0],bbox[1],bbox[2],bbox[3])

        # Draw the label
        pygame.draw.rect(bb_surface, color, pygame.Rect(bbox[0], bbox[1] - 30, 25 * len(detection['label']), 30))

        # label_dawing_operations.append(
        #    lambda: self.display.blit(self.bb_font.render(detection['label'], True, (0, 0, 0)), (bbox[0] + 5, bbox[1] - 30)))
        label_surface.blit(camera_manager.bb_font.render(detection['label'], True, (0, 0, 0)), (bbox[0] + 5, bbox[1] - 30))
        ...
        if show_distance:
            #Draw the distance
            label_surface.blit(camera_manager.bb_font.render(get_distance_by_camera(bbox), True, (0, 0, 0)), (bbox[2] + 15, bbox[3] - 30))

        # Draw the confidence
        # pygame.draw.rect(conf_surface, color, pygame.Rect(bbox[0], bbox[3], bbox[2] - bbox[0], (bbox[1] - bbox[3]) * detection['score']))
            ...
            if args.integrate_object_detector:
                if world.camera_manager.surface is not None:
                    ...
                    if args.fault_type != 'none':
                        ...
                        modified_image = corruptions.test_albu(original_image, aug_type=args.fault_type)
                        ...
                        detections = detect_objects(original_image, 'localhost', 6000)
                    ...
                    draw_bboxes(world, carla, world.camera_manager, display,
                                int(args.res.split('x')[0]), int(args.res.split('x')[1]), detections, args.no_intervention)
